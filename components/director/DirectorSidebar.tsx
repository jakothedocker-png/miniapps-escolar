...
import { GraduationCap, School, LogOut, AlertTriangle, Printer } from 'lucide-react'

const NAV = [
  { href: '/director/escuela',  label: 'Mi Escuela', icon: School },
  { href: '/director/reportes', label: 'Reportes',   icon: Printer },
  { href: '/director/riesgo',   label: 'En Riesgo',  icon: AlertTriangle },
]

export default function DirectorSidebar({ nombre }: { nombre: string }) {
  ...
  return (
    <aside className="w-56 flex-shrink-0 flex flex-col min-h-screen"
      ...
      <div style={{ background: 'linear-gradient(135deg, #0687D8 0%, #3B5BDB 100%)' }}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.70)' }}>Director</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-3" style={{ borderTop: '1px solid rgba(6,135,216,0.10)' }}>
        <div className="px-3">
          <p className="text-xs" style={{ color: '#94A3B8' }}>Director</p>
