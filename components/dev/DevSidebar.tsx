...
import { LayoutDashboard, Building2, Users, Activity, GraduationCap, LogOut, Terminal } from 'lucide-react'

const NAV = [
  { href: '/dev/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/dev/zonas',     label: 'Zonas',     icon: Building2 },
  { href: '/dev/usuarios',  label: 'Usuarios',  icon: Users },
  { href: '/dev/logs-ia',   label: 'Logs IA',   icon: Activity },
]

export default function DevSidebar({ nombre }: { nombre: string }) {
  const pathname = usePathname()

  return (
    <aside className="w-60 flex-shrink-0 flex flex-col min-h-screen"
      style={{
        background: '#FFFFFF',
      ...
      <div style={{ background: 'linear-gradient(135deg, #0687D8 0%, #3B5BDB 100%)' }}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-mono flex items-center gap-1 mt-0.5" style={{ color: 'rgba(255,255,255,0.70)' }}>
              <Terminal size={10} /> DEV CONSOLE
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-3" style={{ borderTop: '1px solid rgba(6,135,216,0.10)' }}>
        <div className="px-3">
          <p className="text-xs" style={{ color: '#94A3B8' }}>Conectado como</p>
          <p className="text-sm font-medium truncate" style={{ color: '#1E2D3D' }}>{nombre}</p>
          <span className="text-xs px-2 py-0.5 rounded-full font-mono"
            style={{ background: 'rgba(6,135,216,0.10)', color: '#0687D8', border: '1px solid rgba(6,135,216,0.20)' }}>
            superadmin
          </span>
        </div>
        <form action={logout}>
