...
export default function RiesgoSupervisorClient({ alumnos }: Props) {
  const [trimestre, setTrimestre] = useState(0)

  const filtrados = alumnos.filter(a => {
    if (trimestre === 0) return true
    if (trimestre === 1) return a.t1_prom !== null && a.t1_prom < 6
    if (trimestre === 2) return a.t2_prom !== null && a.t2_prom < 6
    if (trimestre === 3) return a.t3_prom !== null && a.t3_prom < 6
    return true
  })

  return (
    <div className="p-6 max-w-7xl mx-auto" style={{ background: '#F5F8FF', minHeight: '100%' }}>
      <div className="mb-6">
        ...
        <p className="text-sm mt-1" style={{ color: '#64748B' }}>
          Alumnos en toda la zona con promedio menor a 6 o con 10 o más inasistencias.
        </p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(6,135,216,0.08)' }}>
                  {['#', 'Alumno', 'Escuela', 'Maestro', 'Grupo', 'T1', 'T2', 'T3', 'Faltas', 'Alerta'].map(h => (
                    <th key={h} className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wide whitespace-nowrap"
                ...
                {filtrados.map((a, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 font-medium whitespace-nowrap" style={{ color: '#1E2D3D' }}>{a.nombre}</td>
                      <td className="px-4 py-3.5 max-w-[160px] truncate" style={{ color: '#64748B' }}>{a.escuela}</td>
                      <td className="px-4 py-3.5 whitespace-nowrap" style={{ color: '#64748B' }}>{a.maestro}</td>
                      <td className="px-4 py-3.5 whitespace-nowrap" style={{ color: '#64748B' }}>{a.grado} &quot;{a.grupo}&quot;</td>
                      <td className="px-4 py-3.5">{celdaRiesgo(a.t1_prom)}</td>
                      <td className="px-4 py-3.5">{celdaRiesgo(a.t2_prom)}</td>
                      <td className="px-4 py-3.5">{celdaRiesgo(a.t3_prom)}</td>
                      <td className="px-4 py-3.5">
                        <span style={{ color: faltasAltas ? '#B45309' : '#1E2D3D', fontWeight: faltasAltas ? 600 : 400 }}>
                          {a.total_faltas}
                        </span>
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {promBajo && (
                            <span className="text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap"
                              style={{ background: '#FEF2F2', color: '#DC2626' }}>Prom. bajo</span>
                          )}
                          {faltasAltas && (
                            <span className="text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap"
                              style={{ background: '#FFFBEB', color: '#B45309' }}>Faltas</span>
                          )}
                        </div>
                      </td>
                    </tr>
